...
fora-do-ar/               página preta, sw.js de saída e robots.txt que o build publica no lugar do site com --fora-do-ar
...
O workflow `.github/workflows/sync-estoque.yml` é um vigia contínuo: cada execução dura quase 6 horas (o máximo de um job), começa com a sincronização completa (`scripts/sincronizar.sh`: estoque, páginas, miniaturas, YouTube, Instagram, validação e commit) e depois, a cada 5 minutos, confere só a listagem e a home (`node scripts/sync-inventory.mjs --check-only`: duas requisições, comparando ids, capas, preços e destaques com `data/index.json`). Quando algo muda, faz a sincronização completa e commita; ao fim do turno, dispara a próxima execução. O workflow `vigia-estoque.yml` confere de hora em hora se há um vigia ativo e inicia um se não houver. Um cron de 5 em 5 minutos não serviria: o agendador do GitHub atrasa muito (a execução diária das 09:00 UTC chegou a rodar às 13:24). Tudo pode ser disparado à mão em **Actions → Sincronizar estoque → Run workflow**. Os commits vão para o branch padrão e o Vercel publica em seguida. Actions é gratuito em repositório público.
...
O repositório já está pronto: `vercel.json` define o build (`node scripts/build-vercel.mjs`; enquanto o site estiver fora do ar, com a opção `--fora-do-ar`, ver abaixo) e a pasta publicada (`dist/`). Não há dependências a instalar. Só falta conectar o repositório a um projeto no Vercel, o que exige a conta do dono:
...
**Tirar o site do ar:** em `vercel.json`, troque o `buildCommand` por `node scripts/build-vercel.mjs --fora-do-ar` e faça push. O build passa a publicar só a pasta `fora-do-ar/`: uma página preta em `index.html` e em `404.html` (portanto em qualquer endereço), um `sw.js` que apaga o cache e desliga o service worker em quem já visitou o site (publicado sem comentários: nada no ar explica a saída), e um `robots.txt` bloqueando tudo. Nada do site (páginas, `assets/`, `data/`, `v/`) vai para o ar. Para saber o estado atual, olhe o `buildCommand` em `vercel.json`. Para voltar, tire `--fora-do-ar` do `buildCommand` e faça push; o deploy seguinte publica o site completo de novo. Os testes cobrem os dois modos. Enquanto o site está fora do ar, convém deixar desativado o workflow `pages.yml` (Actions), que publicaria o site completo no GitHub Pages se fosse disparado à mão. Ressalva na volta: quem estava com o site aberto na hora da retirada pode ter guardado, pelo `Cache-Control` de `/assets/*`, um 404 de miniatura por até 7 dias; passa sozinho.
...
Cuidado com o Google: pop-ups que cobrem o conteúdo logo ao abrir a página podem ser penalizados nas buscas no celular ("interstitials intrusivos"). Por isso o pop-up só aparece na home, só uma vez por vídeo, depois de 1,4 s (e, na primeira visita, só depois de o aviso de cookies ser respondido, para não empilhar duas janelas) e fecha com um toque ou com Esc; nas páginas de veículos (para onde vão os anúncios e os links compartilhados) ele não aparece.
...
- Analytics: já incluído com os mesmos IDs do site atual (GA4 `G-F35L06L32H` e Pixel `410840736561439`, em `assets/js/store.js`). O aviso de cookies tem "Aceitar" e "Só o necessário": sem aceite, o GA4 roda em Consent Mode (sem cookies) e o Pixel não carrega; a política de privacidade tem o botão "Rever minha escolha de cookies", que apaga a escolha e mostra o aviso de novo. Para desligar tudo, deixe os dois IDs como `null`.
- Google Ads: as contas (`AW-17949964372`, com as conversões, e `AW-17947810458`) e os rótulos de conversão vieram do Tag Manager do site atual (contêiner `GTM-59BH7PH9`, público). Aqui a conversão dispara só no clique real em WhatsApp (`contato_whatsapp`) e no envio de formulário (`envio_formulario`), não a cada página como lá. Confirme com quem cuida das campanhas se esses rótulos são os certos; para desligar, deixe `googleAds`/`conversoes` como `null` em `store.js`.
- Fonte Inter hospedada no próprio site (`assets/fonts/inter-latin.woff2`, licença SIL OFL, texto em `assets/fonts/OFL.txt`, arquivo variável do Google Fonts): nenhuma conexão com o Google só para carregar a fonte.
- Endereço provisório: enquanto o site é servido em `*.vercel.app`, `gerar-paginas.mjs` põe `noindex` nas páginas e `Disallow: /` no robots.txt, para o Google não guardar um endereço que vai mudar. Com o domínio próprio configurado no Vercel, isso some sozinho no build seguinte.
- `vercel.json` redireciona as URLs do site atual (`/Veiculos`, `/Veiculo/<slug>/<id>/detalhes`, `/VendaSeuVeiculo`, `/Empresa`, `/Contato`, `/Financiamento`, `/PoliticaPrivacidade`) para as páginas novas, para quando o domínio for apontado para cá; os ids dos anúncios são os mesmos nos dois sites.
- `estoque.html` e a home trazem no HTML uma lista simples com os links de todos os veículos (gerada por `gerar-paginas.mjs`), que o JavaScript troca pelos cartões: os anúncios ficam visíveis para robôs e sem JS.
