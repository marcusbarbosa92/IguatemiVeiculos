...
O workflow `.github/workflows/sync-estoque.yml` confere a listagem a cada 5 minutos (`node scripts/sync-inventory.mjs --check-only`: só duas requisições, comparando ids, capas, preços e destaques com `data/index.json`) e, quando algo mudou, faz a sincronização completa; todo dia às 06:00 (Brasília) faz a completa de qualquer jeito, para pegar fotos e opcionais editados em anúncios que já existiam. O agendamento do GitHub pode atrasar alguns minutos em horários de pico. Também pode ser disparado manualmente em **Actions → Sincronizar estoque → Run workflow**. Ele commita as mudanças no branch padrão; o Vercel reage ao push e publica o estoque novo sozinho.
